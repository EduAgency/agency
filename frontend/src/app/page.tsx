      ...
      <header className="border-b border-line">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-6 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="font-display block text-xl leading-none font-extrabold tracking-tight text-ink">Nasuru</span>
          ...
          <nav aria-label="Main" className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
            <a href="#what-we-do" className="text-muted hover:text-ink">What we do</a>
            <a href="#costs" className="text-muted hover:text-ink">Real costs</a>
            <a href="#fees" className="text-muted hover:text-ink">Our fee</a>
            <Link href="/login" className="font-medium text-ink hover:underline">Sign in</Link>
            <Link href="/signup" className="rounded-lg bg-accent px-4 py-2.5 text-sm font-semibold text-on-accent transition hover:bg-accent-hover">
              Create an account
            </Link>
          </nav>
        </div>
      ...
      <main>
        ...
        <section className="mx-auto max-w-6xl px-6 py-16 text-center sm:py-20">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-muted">
            We find the tuition-free schools that will take your qualifications, tell you exactly what each one
            requires, write your CV and motivation letter with you, and consult on the visa once you have an offer.
          </p>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
            <Link href="/signup" className="rounded-lg bg-accent px-7 py-3.5 text-base font-bold text-on-accent transition hover:bg-accent-hover">
              See my matched schools — {ACCESS_FEE.formatted} once
            </Link>
            ...
            <div>
              ...
              {accreditationPending ? (
                ...
                <p className="mt-1 text-sm leading-relaxed text-muted">
                  ...
                  <a href={ACCREDITATION.verifyUrl} className="text-ink underline underline-offset-2">verify it independently</a>.
                </p>
              )}
            </div>
          </div>
        ...
        <section id="what-we-do" className="border-y border-line bg-sunken py-16">
          <div className="mx-auto max-w-6xl px-6">
            ...
            <p className="mt-3 max-w-3xl text-lg text-muted">
              Not &ldquo;we handle everything&rdquo;. You will write your own motivation letter — with us, until it is
              right. An agent who wants you out of the process is usually hiding how little of it there is.
            </p>
            <ol className="mt-8 grid gap-px overflow-hidden rounded-xl border border-line bg-line sm:grid-cols-2">
              {SERVICES.map((service, index) => (
                <li key={service.title} className="bg-surface p-7">
                  <p className="font-mono text-sm font-bold text-accent">{String(index + 1).padStart(2, "0")}</p>
          ...
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              ...
              <p className="mt-3 leading-relaxed text-muted">
                Most people arrive asking us about visas. That conversation cannot start until a university has
                accepted you, so almost everything we do together happens before it — and the quality of your
                application is the part you can still change.
              </p>
            </div>
            <ol className="grid gap-5 sm:grid-cols-2">
              {ORDER.map((stage) => (
                <li key={stage.title} className="rounded-xl border border-line p-6">
                  <p className="text-xs font-bold tracking-[0.14em] text-accent uppercase">{stage.step}</p>
        ...
        <section className="border-y border-line bg-sunken py-16">
          <div className="mx-auto max-w-3xl px-6 text-center">
            ...
            <p className="mt-4 text-lg leading-relaxed text-muted">
              A fair question, and one you should ask every agent. The shortlist is the work — knowing which
              universities charge no tuition, which of those are any good in your subject, and which will accept a
              Nigerian qualification. Publishing it here would be publishing the thing you are paying for.
            </p>
            <p className="mt-4 text-lg leading-relaxed text-muted">
            ...
            <div className="mt-6 rounded-xl bg-success-bg p-6 text-left text-success">
              <p className="font-display text-lg font-bold">
                Pay {ACCESS_FEE.formatted}. See your matched schools and what each one requires. If it is not worth
                it, ask for your money back within {REFUND.coolingOffDays} days.
              </p>
              <p className="mt-3 leading-relaxed">
                No reason needed, and we will not try to talk you out of it. The only condition is that we have not
                already reviewed one of your documents — and your checklist shows you exactly when that happens.
              </p>
            </div>
            <p className="mt-6 text-muted">
              That is {ACCESS_FEE.formatted} to find out, against a decision worth years of your life.
            </p>
          </div>
        </section>
        ...
        <section id="costs" className="mx-auto max-w-6xl px-6 py-16">
          <div className="grid gap-10 lg:grid-cols-[1fr_1.2fr] lg:items-start">
            <div>
              ...
              <p className="mt-3 leading-relaxed text-muted">
                No tuition is a genuine saving — usually the largest single one available to a Nigerian student. It is
                not the whole bill. You should hear these figures from us now rather than discover them weeks before
                an intake.
              </p>
              <p className="mt-4 leading-relaxed text-muted">
                None of this money comes to us. We tell you what each item costs before you commit to it.
              </p>
            </div>
            <div className="overflow-x-auto rounded-xl border border-line" tabIndex={0} role="region" aria-label="Costs a tuition-free place does not remove">
              <table className="w-full min-w-[30rem] text-left">
                <caption className="sr-only">Costs a tuition-free place does not remove.</caption>
                <thead>
                  <tr className="border-b border-line bg-sunken">
                    <th scope="col" className="px-5 py-3 text-sm font-semibold text-ink">You will still pay for</th>
                    <th scope="col" className="px-5 py-3 text-sm font-semibold text-ink">Roughly</th>
                  </tr>
                </thead>
                <tbody>
                  {REAL_COSTS.map((cost) => (
                    <tr key={cost.item} className="border-b border-line last:border-0">
                      ...
                      <td className="px-5 py-4 align-top">
                        {/* A stale figure here is worse than none, so an
                            unchecked one says so rather than leaking the raw
                            placeholder into the page. */}
                        {isPending(cost.amount) ? (
                          <span className="rounded-full bg-warning-bg px-2.5 py-1 text-xs font-semibold whitespace-nowrap text-warning">
                            Ask us — figures change
                          </span>
                        ) : (
          ...
          <div className="mx-auto max-w-5xl px-6">
            <div className="text-center">
              ...
              <p className="mx-auto mt-3 max-w-2xl text-lg text-muted">
                Every document, its status, and — when something is rejected — exactly why, in writing. Not a monthly
                phone call where you are told it is progressing.
              </p>
            </div>
            ...
            <div className="mt-10 overflow-hidden rounded-t-2xl border border-line bg-surface shadow-2xl">
              ...
              <div className="px-6 py-7 sm:px-8">
                <div className="flex flex-wrap items-end justify-between gap-4">
                  <div>
                    <h3 className="font-display text-xl font-bold text-ink">MSc International Business</h3>
                    <p className="mt-0.5 text-sm text-muted">Public university, no tuition · Collecting documents</p>
                  </div>
                ...
                <div role="img" aria-label="9 of 17 documents verified, 53 percent" className="mt-3 h-2 w-full overflow-hidden rounded-full bg-sunken">
                ...
                <ul className="mt-6 divide-y divide-line overflow-hidden rounded-xl border border-line">
                  <li className="flex items-start gap-4 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-sm text-muted">Written with your counsellor, specific to this programme.</p>
                    </div>
                    <span className="rounded-full bg-success-bg px-2.5 py-1 text-xs font-semibold whitespace-nowrap text-success">Verified</span>
                  </li>
                  ...
                  <li className="flex items-start gap-4 bg-warning-bg p-4">
                    ...
                    <span className="rounded-full bg-surface px-2.5 py-1 text-xs font-semibold whitespace-nowrap text-warning">Needs redoing</span>
                  </li>
                  ...
                  <li className="flex items-start gap-4 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-sm text-muted">Sworn translator, accepted by the university.</p>
                    </div>
                    <span className="rounded-full bg-info-bg px-2.5 py-1 text-xs font-semibold whitespace-nowrap text-info">In review</span>
                  </li>
                </ul>
              </div>
            </div>
        ...
        <section id="fees" className="border-t border-line bg-sunken py-16">
          <div className="mx-auto max-w-6xl px-6">
            <div className="grid items-start gap-12 lg:grid-cols-[1fr_1.1fr]">
              <div>
                <p className="text-xs font-bold tracking-[0.14em] text-accent uppercase">What we charge</p>
                ...
                <div className="mt-6 rounded-xl border border-line bg-surface p-5">
                  <p className="leading-relaxed text-muted">
                    We take no commission from any university, so the shortlist you get is the one that suits you
                    rather than the one that pays us.{" "}
                ...
                <p className="mt-5 leading-relaxed text-muted">
                  Our fee is the only money that comes to us. Application fees, tests, deposits and visa charges are
                  paid by you, directly to those bodies — we never take a cut of any of them.
                </p>
                <p className="mt-4 text-sm text-subtle">
                  Read the <Link href="/refund-policy" className="text-ink underline underline-offset-2">refund policy</Link>{" "}
        ...
        <section className="border-t border-line bg-ink py-16 text-center" style={{ color: "var(--canvas)" }}>
          <div className="mx-auto max-w-3xl px-6">
            ...
            <p className="mx-auto mt-3 max-w-xl text-lg opacity-80">
              Ten minutes, and you will know which tuition-free universities are realistic for you — or that they are
              not, which is worth knowing too.
            </p>
            <Link href="/signup" className="mt-8 inline-block rounded-lg bg-accent px-8 py-3.5 text-base font-bold text-on-accent transition hover:bg-accent-hover">
              Create your account
            </Link>
